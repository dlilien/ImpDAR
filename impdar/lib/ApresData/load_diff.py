...
from . import ApresData, ApresDiffData
from ..ImpdarError import ImpdarError


def load_diff(fn, load_single_pol=True, *args, **kwargs):
    """

    """

    if not load_single_pol:
        diff_data = ApresDiffData(fn)
    else:
        # Load each of the individual polarizations
        ...
        diff_data = ApresDiffData(None)
        diff_data.snum = dat1.snum
        diff_data.data = dat1.data.flatten().astype(np.complex)
        diff_data.data2 = dat2.data.flatten().astype(np.complex)
