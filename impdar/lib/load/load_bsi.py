...
def _xmlGetVal(xml, name):
    """ Look up a value in an XML fragment. Modified from Nat Wilson's irlib"""
...
def load_bsi(fn_h5, *args, **kwargs):
    ...
    # This is different from most input filetypes in that we can have multiple lines
    # stored in one file. We want to preserve these since concatenation may be illogical
    h5_data_list = []
    ...
    with h5py.File(fn_h5) as f_in:
        ...
        for dset_name in dset_names:
            ...
            digitizer_data = dset['location_0']['datacapture_0']['echogram_0'].attrs['Digitizer-MetaData_xml'].decode('utf-8')
            h5_data.dt = 1.0 / float(_xmlGetVal(digitizer_data, ' Sample Rate'))
