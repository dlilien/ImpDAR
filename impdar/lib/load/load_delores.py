...
def load_delores(fn_del, channel=1, *args, **kwargs):
    ...
    with h5py.File(fn_del) as f_in:
        ...
        for i,tr in enumerate(tr_names):
            ...
            if not hasattr(h5_ch[tr].attrs['Time'],'__len__'):
                ...
                date = datetime.date(time_array[0],time_array[1],time_array[2])
                time = time_array[3] + (time_array[4] + (time_array[5] + time_array[6]/1e6)/60.)/60.
    ...
    del_data.trace_int = np.hstack((np.array(np.nanmean(np.diff(del_data.dist))),
                                   np.diff(del_data.dist)))
    """
